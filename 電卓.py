...
def button_action1():  # 関数の定義 ※ボタンが押されたときの動き
    ...
    user_input2 = entry2.get()  # 入力値を取得
    total = int(user_input1) + int(user_input2)
    label1.config(text=f"{user_input1}+{user_input2}={total}")  # 画面に出力


def button_action2():  # 関数の定義 ※ボタンが押されたときの動き
    user_input1 = entry1.get()
    user_input2 = entry2.get()  # 入力値を取得
    total2 = int(user_input1) - int(user_input2)
    label1.config(text=f"{user_input1}-{user_input2}={total2}")  # 画面に出力


def button_action4():  # 関数の定義 ※ボタンが押されたときの動き
    user_input1 = entry1.get()
    user_input2 = entry2.get()  # 入力値を取得
    total3 = int(user_input1) / int(user_input2)
    label1.config(text=f"{user_input1}÷{user_input2}={total3}")  # 画面に出力
